...
class DepositController extends Controller
{
    ...
    protected function depositData($scope = null, $summery = false)
    {
        ...
        if ($scope) {
            $deposits = Deposit::$scope()->with(['user', 'gateway', 'order']);
        } else {
            $deposits = Deposit::with(['user', 'gateway', 'order']);
        }
    ...
    public function approve($id)
    {
        ...
        $order   = Order::where('id', $deposit->order_id)->where('payment_status', 1)->firstOrFail();
