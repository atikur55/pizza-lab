...
class AutomaticGatewayController extends Controller
{
    ...
    public function update(Request $request, $code)
    {
        ...
        if ($request->has('currency')) {
            ...
            foreach ($request->currency as $key => $currency) {
                ...
                $filename = null;
                $existingCurrency = $gateway->currencies()->where('currency', $currency['currency'])->first();
                if ($existingCurrency) {
                    $filename = $existingCurrency->image;
                }
                $uploadedImage = 'currency.' . $key . '.image';
                if ($request->hasFile($uploadedImage)) {
                    try {
                        $filename = fileUploader($request->file($uploadedImage), $path, $size);
                    } catch (\Exception $exp) {
                        $notify[] = ['error', $currencyIdentifier . ' couldn\'t upload the image'];
    ...
    public function remove($id)
    {
        ...
        fileManager()->removeFile(fileManager()->gateway()->path . '/' . $gatewayCurrency->image);
    ...
    public function gatewayCurrencyValidator(Request $request, Gateway $gateway)
    {
        ...
        if ($request->has('currency')) {
            foreach ($request->currency as $key => $currency) {
                ...
                $validationRule['currency.' . $key . '.image']          = ['nullable', 'image', new FileTypeValidate(['jpg', 'jpeg', 'png'])];
                ...
                $customAttributes['currency.' . $key . '.image']          = $currencyIdentifier . ' ' . keyToTitle('image');
